...
export const showEditModal = (type, record) => {
  return new Promise((resolve) => {
    ...
    const getInputsHTML = () => {
      ...
      if (type === "animal") return `<label>Tipo: <input type="text" name="type" value="${escapeHtml(record.type)}" required></label><label>Status: <input type="text" name="status" value="${escapeHtml(record.status)}" required></label>`;
