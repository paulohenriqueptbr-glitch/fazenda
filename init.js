...
const initApp = () => {
  ...
  if (!document.body._cropGroupTabsAttached) {
    document.body._cropGroupTabsAttached = true;
    document.addEventListener("click", (event) => {
      const cropTab = event.target.closest("[data-crop-group]");
      if (!cropTab) return;

      selectedCropGroup = cropTab.dataset.cropGroup;
      const cropGroupSelect = $("#cropGroup");
      if (cropGroupSelect && CROP_GROUPS.includes(selectedCropGroup)) {
        cropGroupSelect.value = selectedCropGroup;
      }
      renderCropEvents();
    });
  }
  ...
  if (el.cropForm) {
    el.cropForm.addEventListener("submit", withButtonLoading(el.cropForm, async (event) => {
      ...
      try {
        const cropGroup = $("#cropGroup").value.trim();
  ...
  if ($("#cropGroup") && !selectedCropGroup) {
    selectedCropGroup = DEFAULT_CROP_GROUP;
    $("#cropGroup").value = selectedCropGroup;
  }
  if (el.reminderDate) el.reminderDate.value = todayIso();
  if (el.weatherCity) {
