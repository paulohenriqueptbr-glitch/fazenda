...
const normalizeCropEventInput = (data) => {
  ...
  const cropGroup = CROP_GROUPS.includes(cropGroupRaw) ? cropGroupRaw : DEFAULT_CROP_GROUP;
  ...
  return new Promise((resolve) => {
    ...
    const getInputsHTML = () => {
      if (type === "milk") {
        ...
        const currentGroup = CROP_GROUPS.includes(record.crop_group) ? record.crop_group : DEFAULT_CROP_GROUP;
        return `
          <label>Lavoura: <select name="crop_group">
            ${CROP_GROUPS.map((group) => `<option value="${escapeHtml(group)}" ${group === currentGroup ? "selected" : ""}>${escapeHtml(group)}</option>`).join("")}
          </select></label>
...
const editRecord = async (type, id) => {
  ...
  try {
    if (type === "milk") {
      ...
      const cropGroupRaw = (data.crop_group || "").trim();
      const cropGroup = CROP_GROUPS.includes(cropGroupRaw) ? cropGroupRaw : DEFAULT_CROP_GROUP;
